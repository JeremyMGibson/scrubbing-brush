...
class MeasureService {
	...
	def  runMeasure(Measure measure) {
		...
		if (measure.updateScript is not empty && run.success) {
			runUpdate(measure)
	...
	def executeSql(Connection connection, String query) {
		//Load connection
      		Connection conn = Class.forName("com.mysql.jdbc.Driver") 		conn = DriverManager.getConnection("jdbc:mysql://localhost/mydb", "root", "root")
	 	Statement stmt = conn.createStatement()
		return stmt.executeQuery(measure.query)
	}
}